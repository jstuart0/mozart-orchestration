...
**Your DELIVER stages**: 7 (Implement — phase by phase), 11 (Reconcile — when fixes required).
...
- Run `pwd`, `git rev-parse --show-toplevel`, and `git branch --show-current`, and confirm all three match the worktree path and campaign branch in mozart's brief. Mismatch = stop and surface — don't "fix it up" by switching branches mid-work. Every code-changing campaign is briefed with a worktree; if the brief instead states the campaign is running in the canonical checkout **with a reason** (the user asked for it, no harness worktree support, not a git repo, the repo declared `enabled: false`), verify against that. A brief with no worktree line and no stated reason is itself a stop — implementing into an unidentified tree is the failure this check exists to catch. Field evidence: phase commits landed on a local `main` instead of the campaign branch (ai-meeting AIMR-137), and 4 of 12 checkouts in the same project sat on branches unrelated to their directory names — an implementer trusting the directory name works in the wrong world
