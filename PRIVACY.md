...
- **Web requests** — some agents (sarah, web-search-researcher, scott, nina) perform web searches and fetches via WebSearch and WebFetch.
- **Cloud provider read APIs** — **only if** you dispatch nina, the cloud specialist, and credentials are already present on the machine. Two different things happen on that path and they are worth separating. Her *documentation* fetches are ordinary unauthenticated reads of a provider's public pages, the same shape as any other agent's WebFetch. Her *live reads* are authenticated calls against your own account, and they are narrower in every direction: **live-read mode is AWS-only** — on Azure and GCP she applies the same method with no live reads at all — and it is granted by the conductor at dispatch when the brief names an operator-declared principal, never self-selected by the agent. A call runs only when **all four** of an allowed verb, a non-denied bucket, a declared leaf-path projection, and an exact-ARN principal match hold together; fail any one and the call is refused and recorded as `[unresolved]` rather than skipped silently. The projection rule is an **allowlist** — a path is admissible only if it matches a fixed grammar and carries a declared value kind from a closed set of seven — so a call that reaches data is refused because nothing admitted it, not because someone remembered to forbid it. **She never transmits repository content, plan text, or diffs to a provider.** What leaves the machine is the API call itself, attributable to the credential you supplied and visible in that provider's own audit trail like any other call made with it.
...
**A second enforcement layer for the cloud path, optional and yours to apply.** `tests/policy/nina-review-role.json` is a deny-by-default IAM skeleton you adapt into a real review role, so the refusals above also exist in the provider rather than only in the persona's instructions. It is partial coverage by construction and is defence in depth behind the projection rule, never a substitute for it. **That file ships in the repository and a flat user-scope install does not carry it**: copying `agents/` into `~/.claude/agents/` (`CONTRIBUTING.md:77`) copies agent markdown and nothing else, so a JSON file under `tests/` never arrives. Fetch it from the repository at `tests/policy/nina-review-role.json`.
...
- Persist any data outside of your local working directory — **except on one path you must explicitly enable**: a `## Pull requests` stanza with `enabled: true` in your `CLAUDE.md` authorizes DELIVER stage 12b to push the campaign branch and open a pull request on **your own configured remote**. That is the only way the plugin writes anything off your machine, it is off unless you declare it, and it goes nowhere but the remote your repo already points at. Delete the stanza and the behavior is gone. nina's provider calls do not change this: they read configuration and write nothing back, though the fact that a call was made is recorded in your provider's own audit trail, as it would be for any other use of those credentials.
...
- Your cloud provider (AWS, Azure, GCP), if you dispatch nina against an account: see that provider's policy
