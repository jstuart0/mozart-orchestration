...
1. **YAML frontmatter** — `name`, `description`, `tools`, `model`. The `description` field is what the Claude Code marketplace shows; write it for an external reader who doesn't know the pipeline. ~30–50 words. No jokes. A persona's `tools:` line must satisfy every capability its own contracts promise — a persona file must not claim it writes an artifact, or spawns another agent, that its `tools:` line cannot deliver. This is enforced mechanically by `scripts/mozart-contract-gates.sh`; see that script for what it checks, not restated here.
